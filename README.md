...
### 🚕 ลูกบ้าน - บริการรถเรียก

```mermaid
flowchart TD
    A[ลูกบ้านเข้าสู่ระบบ] --> B[ไปหน้าบริการรถเรียก]
    B --> C[เลือกประเภทบริการ]
    C --> D[แท็กซี่]
    C --> E[มอเตอร์ไซค์รับจ้าง]
    D --> F[กรอกจุดรับ-จุดหมาย]
    E --> F
    F --> G[เลือกเวลารับ<br/>- ทันที<br/>- จองล่วงหน้า]
    G --> H[ระบบแสดงค่าโดยสารโดยประมาณ]
    H --> I[กดยืนยันเรียกรถ]
    I --> J[สถานะ: กำลังหาคนขับ]
    J --> K{พบคนขับ?}
    K -->|ใช่| L[แสดงข้อมูลคนขับ<br/>- ชื่อ<br/>- ทะเบียนรถ<br/>- เบอร์โทร]
    K -->|ไม่| M[แจ้งไม่พบคนขับ ลองใหม่]
    M --> I
    L --> N[สถานะ: คนขับกำลังมา]
    N --> O[คนขับถึงหน้าหมู่บ้าน]
    O --> P[รปภ. ยืนยันให้เข้า]
    P --> Q[รับลูกบ้านขึ้นรถ]
    Q --> R[สถานะ: กำลังเดินทาง]
    R --> S[ถึงจุดหมาย]
    S --> T[สถานะ: เสร็จสิ้น]
```
...
### 🎫 ลูกบ้าน - ติดต่อนิติบุคคล (Support)

```mermaid
flowchart TD
    A[ลูกบ้านเข้าสู่ระบบ] --> B[ไปหน้าติดต่อนิติ]
    B --> C[กดสร้างคำร้องใหม่]
    C --> D[เลือกหัวข้อ<br/>- ร้องเรียน<br/>- สอบถาม<br/>- แจ้งปัญหา]
    D --> E[กรอกรายละเอียด + แนบรูป]
    E --> F[กดส่งคำร้อง]
    F --> G[สถานะ: เปิดใหม่]
    G --> H[Admin ได้รับแจ้งเตือน]
    H --> I[Admin ตอบกลับ]
    I --> J[สถานะ: กำลังดำเนินการ]
    J --> K[ลูกบ้านได้รับแจ้งเตือน]
    K --> L[ลูกบ้านตอบกลับ]
    L --> I
    I --> M{ปัญหาจบ?}
    M -->|ใช่| N[Admin ปิดงาน]
    M -->|ไม่| I
    N --> O[สถานะ: ปิดงาน]
```
...
```mermaid
flowchart TD
    A[มีพัสดุมาส่ง] --> B[รปภ. บันทึกเข้าระบบ]
    B --> C[ลูกบ้านได้รับแจ้งเตือน]
    C --> D[ลูกบ้านไปที่ป้อม รปภ.]
    D --> E[เปิด App หน้าพัสดุ]
    E --> F[แสดงรายการพัสดุค้างรับ]
    F --> G[แจ้งห้อง/ชื่อ กับ รปภ.]
    G --> H[รปภ. ตรวจสอบและส่งมอบ]
    H --> I[รปภ. กดปิดงานในระบบ]
    I --> J[สถานะ: รับแล้ว]
    J --> K[ประวัติพัสดุอัพเดท]
```
...
flowchart TD
    ...
    B -->|พัสดุใหม่| C[สร้าง Notification]
    B -->|บิลใหม่| C
    B -->|งานซ่อมเปลี่ยนสถานะ| C
    B -->|ตอบกลับ Support| C
    B -->|ประกาศใหม่| C
    C --> D[บันทึกลง Database]
    D --> E[ส่งไป OneSignal API]
    E --> F[OneSignal ส่งไปที่เครื่องผู้ใช้]
    F --> G[ผู้ใช้เห็น Notification]
    G --> H[ผู้ใช้กดที่แจ้งเตือน]
    H --> I[เปิด App]
    I --> J[Redirect ไปหน้าที่เกี่ยวข้อง]
```

---

### 🎛️ Admin - ตั้งค่า Feature Toggles

```mermaid
flowchart TD
    A[Admin เข้าสู่ระบบ] --> B[ไปหน้าตั้งค่าฟีเจอร์]
    B --> C[ดูรายการฟีเจอร์ทั้งหมด]
    C --> D[เลือกเปิด/ปิดฟีเจอร์]
    D --> E[กดบันทึก]
    E --> F[ระบบอัพเดทฐานข้อมูล]
    F --> G{ผลกระทบ}
    G --> H[ซ่อนเมนูใน Sidebar]
    G --> I[บล็อกหน้าที่เกี่ยวข้อง]
    G --> J[บล็อก API Endpoints]
    H --> K[ผู้ใช้เห็นเฉพาะฟีเจอร์ที่เปิด]
    I --> K
    J --> K
```

**Feature Toggles ประกอบด้วย:**

| Layer | การป้องกัน | Component |
|-------|-----------|-----------|
| **Sidebar** | ซ่อนเมนู | `FeatureGate`, `FeatureGatedMenuItem` |
| **Page** | บล็อกหน้า | `withFeaturePage` HOC |
| **API** | บล็อก Endpoint | `isFeatureEnabled` check |

**Usage:**
```tsx
// Sidebar Menu
<FeatureGatedMenuItem item={{ featureKey: 'maintenance', ... }} />
